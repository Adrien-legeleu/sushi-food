                            ...
                            <Splide className="w-[85%] mx-auto" options={{ perPage: 3, pagination: false }}>
                                {
                                    menu.repas.map((food, index) => {
                                        return (
                                            <SplideSlide className="rounded-3xl flex flex-col items-center justify-between pl-5 pr-5" key={index}>
                                                <h4 className="text-whiteTitle text-lg font-text">{food.title}</h4>
                                                <img className="rounded-3xl" src={food.src} alt={`image ${food.title}`} />
                                                <p className="text-whiteText font-text text-sm">{food.title}</p>
                                            </SplideSlide>
                                        )
                                    })
                                }
