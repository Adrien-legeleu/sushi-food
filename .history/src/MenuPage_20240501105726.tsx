            ...
            <div className="flex  flex-col gap-10 mt-32">
                {MenuData.map((menu, index) => {
                    return (
                        <div key={index}>
                            <h3>{menu.titre}</h3>
                            <Splide className="flex items-center justify-center w-3/4  gap-5" options={{ perPage: 3   , pagination:false}}>
                                {
                                    menu.repas.map((food , index)=>{
                                        return(
                                            <SplideSlide className="bg-whiteTitle rounded-3xl flex flex-col items-center justify-between" key={index}>
                                                <h4>{food.title}</h4>
